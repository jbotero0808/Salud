import { diasDeLaSemana, isSameDay, format } from './calendarUtils';

export default function WeekView({ fechaRef, citas, onDiaClick, onCitaClick }) {
  const dias = diasDeLaSemana(fechaRef);
  const hoy = new Date();

  return (
    <div className="calendar-week-grid" style={{ gridTemplateColumns: 'repeat(7, 1fr)' }}>
      {dias.map((dia) => {
